...
The R code chunk below is *not* included in the [source package](https://r-pkgs.org/package-structure-state.html#source-package). The chunk has to be executed
at least once to generate the necessary internal data file [`R/sysdata.rda`](https://r-pkgs.org/data.html#data-sysdata).
...
TODO: use obj `pkg_config` instead (cf. `?pal::pkg_config_val`)
...
##### `get_geneva_referendum_dates_skeleton`
...
##### `scrape_geneva_referendum_dates_before_2007`
...
##### `scrape_geneva_referendum_dates_between_2007_2014`
...
scrape_geneva_referendum_dates_between_2007_2014 <- function(year) {
  ...
  referendum_dates %>% dplyr::mutate(municipality = dplyr::if_else(level == "municipal",
                                                                   municipality,
                                                                   NA_character_))
}
```

##### `scrape_geneva_referendum_dates_after_2014`
...
##### `scrape_geneva_referendum_date_between_2014_mid_2018`
...
##### `scrape_geneva_referendum_date_after_mid_2018`
...
convert_canton_names <- function(cantons) {
  ...
  for (canton_name in checkmate::assert_character(cantons,
    ...
    } else cli::cli_abort("{.val {canton_name}} is not a valid e-voting canton.")
  }
