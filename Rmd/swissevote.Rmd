...
There is one municipality present in the raw data, *La Grande Béroche*, which has officially changed its spelling. It was created by the merger of four
municipalities on 2018-01-01 and it was initially foreseen that it would be written *La Grande-Béroche*. But the hyphen [was dropped effective
2018-04-01](https://archive.is/x9ixU). A translation of the relevant info:
...
> municipality of *La Grande Béroche* was created, that its name would include a hyphen. However, the merger agreement did not include a hyphen in the name of
...
We treat this municipality as if its name had always been written without a hyphen: *La Grande Béroche*
...
### `ballot_dates_federal_neuchatel`

Get federal ballot dates from Neuchatel's official website
...
ballot_dates_federal_neuchatel <- function(exclude_counterproposals = FALSE) {
  ...
  ballot_dates <-
    ...
    rvest::html_elements(css = ".SubSectionTitle") %>%
    rvest::html_text() %>%
    stringr::str_squish() %>%
    stringr::str_replace(pattern = "^(.+?)( - \\d+?\\.\\d+?\\.\\d+)( )(\\(.+?\\))$",
                         replacement = "\\1 \\4\\2") %>%
    stringr::str_split_fixed(pattern = " - (?=\\d+?\\.\\d+?.\\d+?$)",
                             n = 2L) %>%
    magrittr::set_colnames(c("subject", "ballot_date")) %>%
    tibble::as_tibble() %>%
    dplyr::mutate(ballot_date = lubridate::dmy(ballot_date),
  ...
  for (i in seq_along(raw_extract)) {
    ...
    if (isTRUE(rvest::html_attr(x = raw_extract[[i]],
      ...
      ballot_dates %<>% tibble::add_row(ballot_date = lubridate::dmy(text_i[2L]),
                                        level = level_i,
                                        municipality = ifelse(level == "municipal",
                                                              text_i[1L] %>%
                                                                stringr::str_extract(pattern = "^.+?(?= - )") %>%
                                                                stringr::str_replace_all(pattern = stringr::fixed("Les Geneveys s/Coffrane"),
                                                                                         replacement = "Les Geneveys-sur-Coffrane"),
                                                              NA_character_),
                                        is_election = stringr::str_detect(string = text_i[1L],
                                                                          pattern = "(?i) - (e|\u00e9)lection"),
                                        subject = ifelse(level == "municipal",
                                                         stringr::str_extract(string = text_i[1L],
                                                                              pattern = "(?<= - ).*$"),
                                                         text_i[1L]))
    }
  }
...
determine_3_code_combo <- function(...) {
  ...
  if (all(is.na(input_codes))) {
    ...
    cli::cli_alert_warning(msg_combined_code_all_na)
...
combine_elected_powers <- function(federal_elected_power = NA,
  ...
  determine_7_code_combo(dplyr::if_else(federal_elected_power > 0L,
                                        1L,
...
#### `get_federal_elected_power_name`
...
#### `get_elected_power_name`

Convert `*_elected_power` code to name(s)

NOTES:

-   This does *not* apply to `federal_elected_power`!
...
##### `get_neuchatel_cantonal_ballot_dates`
...
-   If `council_of_states` is set to `TRUE`, the Neuchatel election dates of the Council of States (federal level) are returned instead of the cantonal ballo
    dates.
...
##### `get_neuchatel_municipal_ballot_dates`
...
-   Note that it might take some time to scrape *all* the 90+ ballot dates (minutes at most).

```{r}
get_geneva_rfrnd_dates <- function(years = 1996:lubridate::year(lubridate::today()),
                                   exclude_counterproposals = FALSE) {
  
  rfrnd_dates <- get_geneva_rfrnd_dates_skeleton()
  
  for (year in years) {
    
    rfrnd_dates %<>% dplyr::bind_rows(get_geneva_rfrnd_date(year = year))
  }
  
  rfrnd_dates %<>%
    # ensure correct spelling of "Gen\u00e8ve"
    dplyr::mutate(municipality = dplyr::if_else(municipality == "Geneve",
                                                "Gen\u00e8ve",
                                                municipality)) %>%
    # arrange columns
    dplyr::select(ballot_date,
...
##### `get_geneva_rfrnd_date`

Gateway to get referendum dates for a specific year

TODO: Meanwhile all the ballot dates were migrated to the new page format (see `scrape_geneva_rfrnd_date_after_mid_2018()`) and all the GE scraping code needs
to be updated.

```{r}
get_geneva_rfrnd_date <- function(year) {
  
  if (year < 2007L) {
    
    rfrnd_dates <- scrape_geneva_rfrnd_dates_before_2007(year = year)
    
  } else if (year < 2015L) {
    
    rfrnd_dates <- scrape_geneva_rfrnd_dates_2007_to_2014(year = year)
    
  } else {
    
    rfrnd_dates <- scrape_geneva_rfrnd_dates_after_2014(year = year)
  }
...
##### `get_geneva_rfrnd_dates_skeleton`

Create empty skeleton of Geneva referendum dates

```{r}
get_geneva_rfrnd_dates_skeleton <- function() {
...
##### `scrape_geneva_rfrnd_dates_before_2007`
...
```{r}
scrape_geneva_rfrnd_dates_before_2007 <- function(year) {
  
  raw_extract <-
    xml2::read_html(x = paste0("https://www.ge.ch/votations/", year, ".asp"),
                    encoding = "windows-1252") %>%
    rvest::html_elements(css = "#texte_bordure") %>%
    rvest::html_elements(css = "ul")
  
  level <-
    raw_extract %>%
    rvest::html_attr(name = "class") %>%
    stringr::str_replace(pattern = stringr::fixed("communal"),
      ...
      rfrnd_dates %<>%
        tibble::add_row(ballot_date =
                        ...
                        subject =
                          ...
                          stringr::str_squish(),
                        municipality =
                          ifelse(level == "municipal",
                                 subject %>%
                                   stringr::str_extract(pattern = ifelse(lubridate::year(ballot_date) != 2006L,
                                                                         "^.+?(?=:)",
                                                                         "(?<=\\s)\\S+$")),
                                 NA_character_))
    }
  ...
  rfrnd_dates %>%
    ...
    dplyr::mutate(municipality = dplyr::if_else(level == "municipal" & lubridate::year(ballot_date) == 2005L,
                                                "Gen\u00e8ve",
                                                municipality))
}
```

##### `scrape_geneva_rfrnd_dates_2007_to_2014`
...
```{r}
scrape_geneva_rfrnd_dates_2007_to_2014 <- function(year) {
  ...
  for (i in seq_along(raw_extract)) {
    ...
    if (is.na(level[i])) {
      ...
      if (length(raw_subextract)) {
        ...
        rfrnd_dates %<>% tibble::add_row(ballot_date = !!ballot_date,
                                         level = !!level[i],
  ...
  rfrnd_dates %>% dplyr::mutate(municipality = dplyr::if_else(level == "municipal",
                                                              municipality,
                                                              NA_character_))
}
```

##### `scrape_geneva_rfrnd_dates_after_2014`
...
```{r}
scrape_geneva_rfrnd_dates_after_2014 <- function(year) {
  ...
  for (i in seq_along(raw_extract)) {
    ...
    if (length(href)) {
      ...
      if (ballot_date < "2018-09-23") {
        ...
        rfrnd_dates %<>% dplyr::bind_rows(scrape_geneva_rfrnd_date_2014_mid_2018(ballot_date = ballot_date,
                                                                                 url = subpage_url))
      } else {
        
        rfrnd_dates %<>% dplyr::bind_rows(scrape_geneva_rfrnd_date_after_mid_2018(ballot_date = ballot_date,
                                                                                  url = subpage_url))
      }
    }
  }
...
##### `scrape_geneva_rfrnd_date_2014_mid_2018`

Scrape a referendum date between 2014 and 2018-06-10

This function scrapes the ballot-date-specific subpages of the old format. The ballot-date-specific subpages from `2018-09-23` onwards are hosted on a new site.

```{r}
scrape_geneva_rfrnd_date_2014_mid_2018 <- function(ballot_date,
                                                   url) {
  raw_extract <-
  ...
  raw_extract %<>% rvest::html_elements(css = "ul")
...
##### `scrape_geneva_rfrnd_date_after_mid_2018`
...
```{r}
scrape_geneva_rfrnd_date_after_mid_2018 <- function(ballot_date,
                                                    url) {
  ...
  objet_selectors <-
    raw_extract %>%
    rvest::html_elements(css = ".container") %>%
    rvest::html_elements(css = "*") %>%
    rvest::html_attr("id") %>%
    unique() %>%
    magrittr::extract(startsWith("objet_") & !is.na(.)) %>%
    paste0("#", .)
  
  vote_locations <-
    raw_extract %>%
    rvest::html_elements(css = ".container") %>%
    rvest::html_elements(css = "h2") %>%
    rvest::html_text() %>%
  ...
  subjects <-
    raw_extract %>%
    rvest::html_elements(css = paste0(objet_selectors, collapse = ","))
  ...
  subjects <-
    ...
    stringr::str_squish() %>%
    paste(subjects %>%
            rvest::html_elements(css = "small") %>%
            rvest::html_text() %>%
            stringr::str_squish(),
          sep = " \u2013 ")
  
  rfrnd_dates <- tibble::tibble(ballot_date = ballot_date,
                                level =
                                  objet_selectors %>%
                                  stringr::str_extract(pattern = "(?<=^#objet_).+?(?=_)") %>%
                                  stringr::str_replace(pattern = stringr::fixed("FED"), replacement = "federal") %>%
                                  stringr::str_replace(pattern = stringr::fixed("CAN"), replacement = "cantonal") %>%
                                  stringr::str_replace(pattern = stringr::fixed("COM"), replacement = "municipal"),
...
-   The relevant website <https://www.ge.ch/elections/> is not of a well-structured enough form to retrieve all the data of interest programmatically. Therefore
    the function below returns a `subject_summary` of the ballot date (providing the information what institutions were elected in varying levels of detail)
    instead of the individual `subject`s (i.e. elected institutions, one per row), and a `link` pointing to the relevant subpage where more info about the
    election date can be found.
...
get_geneva_election_dates <- function() {
  ...
  raw_extract <-
    xml2::read_html(x = "https://www.ge.ch/elections/",
                    encoding = "UTF-8") %>%
    rvest::html_elements(css = ".liste-lien") %>%
    rvest::html_elements(css = "li") %>%
    rvest::html_elements(css = "a") %>%
  ...
  if (election_dates %>%
      ...
      magrittr::not())
  {
    election_dates %<>% dplyr::mutate(ballot_date,
    ...
    cli::cli_abort(paste0("Elections on multiple levels at the same date detected. Please update the code of {.fun get_geneva_election_dates} to support this ",
                          "properly."))
  }
...
##### `get_geneva_municipal_election_dates`
  ...
  for (i in seq_len(nrow(election_dates))) {
    ...
    if (length(matched_municipality) == 0L) {
      ...
      if (length(try_to_match) == 0L) {
        ...
        try_to_match <-
          raw_extract %>%
          rvest::html_elements(css = ".content") %>%
          rvest::html_elements(css = "h2")
      }
      ...
      if (length(matched_municipalities) == 0L) {
        ...
        for (link in subpage_links) {
          ...
          if (length(new_subject_summary) != 1L) {
            ...
            cli::cli_abort("Unexpected scraping result. Please update the code of {.fun get_geneva_municipal_election_dates}.")
      ...
      cli::cli_abort("Multiple municipalities detected in {.var subject_summary}. Please update the code of {.fun get_geneva_municipal_election_dates}.")
    }
...
1.  Only cantonal referendums can be easily scraped. For [cantonal
    elections](https://wahlen-abstimmungen.zh.ch/internet/justiz_inneres/wahlen-abstimmungen/de/wahlen/wahlarchiv.html) there doesn't seem to exist anything
    well structured so far.

2.  Municipal referendums aren't collected centrally anywhere (the canton just [points to the municipal/city
    administrations](https://wahlen-abstimmungen.zh.ch/internet/justiz_inneres/wahlen-abstimmungen/de/fragen-antworten.html)), so they can't be scraped from a
    single site. Instead we have to scrape all the individual sites of the different municipalities (13 currently) to get this data. In some cases (4 currently)
    there's no kind of vote archive available at all we could scrape and in other cases (2 currently) the website is not of a well-structured enough form to
    retrieve all the data of interest programmatically (we just try to return as much as possible).

##### `get_zurich_rfrnd_dates`
...
-   The data can be retrieved as a JSON object from this URL: <http://www.web.statistik.zh.ch/cms_abstimmungsarchiv/includes/ajax_get_vorlagen.php>.
    ...
    -   If you set the parameter `search=true`, the API accepts the following parameters:
        ...
        -   `titel` to search for words occuring in the title of the referendum; multiple words must be separated by `/`; example to search for the words
            `geld     and`spiel`:`titel=geld/spiel\`

        -   `and_search=on` to restrict the results of a `titel` search to those that match *all* words at once.
...
get_zurich_rfrnd_dates <- function(exclude_counterproposals = FALSE) {
  ...
  if (exclude_counterproposals) {
    ...
    rfrnd_dates %<>% dplyr::filter(!stringr::str_detect(string = type,
...
##### `get_zurich_municipal_ballot_dates`

Get municipal ballot dates from the different official Zurich municipal websites

**Remarks:**

-   Set the parameter `exclude_counterproposals` to `TRUE` in order to exclude indirect counterproposals and tie-breaker questions. Note that the exclusion is
    only really implemented yet for the municipality of Zurich because the other municipalities don't show any indirect counterproposals and tie-breaker
    questions up until now (effective July 2018).
...
get_zurich_municipal_ballot_dates <- function(municipalities = NULL,
  ...
  checkmate::assert_subset(municipalities,
  ...
  for (municipality in municipalities) {
    ...
    if (municipality %in% unimplemented_municipalities) {
      ...
      cli::cli_alert_warning(paste0("Scraping ballot dates from the municipality {.val {municipality}} hasn't been implemented yet (or isn't even possible to ",
                                    "implement). Therefore its municipal ballot dates won't be included."))
      
    } else if (municipality %in% partially_implemented_municipalities) {
      
      cli::cli_alert_warning(paste0("Scraping ballot dates from the municipality {.val {municipality}} is only partially implemented so far and the structure ",
                                    "of the returned data differs from the other municipalities. Therefore its municipal ballot dates won't be included."))
    } else {
      
      # print a warning about missing election dates from Zurich
      if (municipality == "Z\u00fcrich") {
        
        cli::cli_alert_warning(paste0("For the municipality {.val {municipality}} only referendum dates can be scraped. The municipal elections dates have to ",
                                      "be gathered by hand."))
      }
...
##### `get_zurich_municipal_dates_boppelsen`

**Remarks**:

-   Due to the bad/ambiguous structure of the website it's rather tricky to identify elections on the level of individual votes and the regex-based strin
    detection we have to rely on is pretty fuzzy. Be sure to check from time to time if the defined regular expression still captures *all* of the elections.
...
get_zurich_municipal_dates_boppelsen <- function() {
  ...
  raw_extract <-
    xml2::read_html(x = "http://www.boppelsen.ch/gemeinde/politik/wahlen/archiv/default.htm",
                    encoding = "windows-1252") %>%
    rvest::html_elements(css = ".portletDocumentListWrapper")
...
##### `get_zurich_municipal_dates_buelach`
...
-   The relevant website <https://www.buelach.ch/themen/politik_verwaltung/wahlen_und_abstimmungen/wahlen_und_abstimmungen_resultate/> doesn't provide the
    ...
    if it was a referendum, election or both) instead of the individual `subject`s, and a `url` pointing to the relevant subpage where more info about the
    ballot date is found.
...
get_zurich_municipal_dates_buelach <- function() {
  ...
  raw_extract <-
    xml2::read_html(x = "https://www.buelach.ch/themen/politik-verwaltung/wahlen-und-abstimmungen/abstimmungen-und-wahlen-resultate",
                    encoding = "UTF-8") %>%
    rvest::html_elements(css = ".element") %>%
    rvest::html_elements(css = ".list") %>%
    dplyr::first() %>%
    rvest::html_elements(css = "ul") %>%
    rvest::html_elements(css = "li")
  ...
  tibble::tibble(ballot_date =
    ...
    dplyr::select(ballot_date, municipality, is_election_date, subject_summary, url) %>%
    dplyr::arrange(ballot_date, municipality, is_election_date)
}
```

##### `get_zurich_municipal_dates_maennedorf`
...
-   The variable `is_election` is determined by some fuzzy regex-based string detection. Be sure to check from time to time if the defined regular expression
    still captures *all* of the elections.

-   The site requires a minimum date passed as a URL parameter (`date_f`). We set it to the date `2000-01-01`, which means this function won't return any dates
    before then.
...
##### `get_zurich_municipal_dates_maur`
...
-   Development note: The web server seems to block the requesting IP address if it detects scraping attempts. In that case, the HTTP error 403 will be
    returned.
  ...
  raw_extract <-
    httr::POST(url = "https://www.maur.ch/politik-verwaltung/politik/wahlen-abstimmungen.html/52/newsarchive/true",
           ...
           body = list(newsSearchParams.query = "",
                       ...
                       newsSearchParams.category = 8L,
                       newsSearchParams.dateFrom = "",
                       newsSearchParams.dateTo = ""),
           encode = "form") %>%
    xml2::read_html(encoding = "UTF-8") %>%
    rvest::html_element(".news-lst-archive") %>%
...
##### `get_zurich_municipal_dates_mettmenstetten`
...
##### `get_zurich_municipal_dates_schlieren`
...
##### `get_zurich_municipal_dates_thalwil`
...
##### `get_zurich_municipal_dates_winterthur`
...
get_zurich_municipal_dates_winterthur <- function() {
  ...
  raw_extract <-
    ...
    rvest::html_elements(css = ".electionday-list") %>%
    rvest::html_elements(css = ".row")
...
##### `get_zurich_municipal_dates_zuerich`
...
get_zurich_municipal_dates_zuerich <- function(exclude_counterproposals = FALSE) {
  ...
  if (exclude_counterproposals) {
    ...
    rfrnd_dates %<>% dplyr::filter(!stringr::str_detect(string = subject,
...
###### `get_zurich_municipal_dates_generic_iweb_1`
...
get_zurich_municipal_dates_generic_iweb_1 <- function(municipality,
  ...
  for (municipal_type in municipal_types) {
    ...
    if (all(c("Termin", "Titel der Vorlage") %in% colnames(raw_extract))) {
      ...
      data <-
        raw_extract %>%
        dplyr::mutate(ballot_date = lubridate::dmy(Termin),
  ...
  if (nrow(data) == 0L) {
    ...
    cli::cli_abort(paste0("No {.code <table>} node of the expected structure was returned. You might have to update the function ",
                          "{.fun get_zurich_municipal_dates_generic_iweb_1}."))
  }
...
get_zurich_municipal_dates_generic_iweb_2 <- function(municipality,
  ...
  dplyr::bind_rows(data[[1L]] %>% dplyr::mutate(is_election = FALSE),
                   data[[2L]] %>% dplyr::mutate(is_election = TRUE)) %>%
...
#### `merge_municipalities_by_canton`
...
merge_municipalities_by_canton <- function(data_canton) {
  ...
  if (nrow(mutations_daughters)) {
    ...
    data_canton %>%
      dplyr::group_by(date) %>%
      dplyr::group_split() %>%
      purrr::map(merge_municipalities_by_canton_date,
...
#### `merge_municipalities_by_canton_date`
  ...
  for (mutation_nr in mutation_nrs) {
    ...
    if (length(ix_daughter)) {
      ...
      merged_rows <-
        ...
        dplyr::group_split() %>%
        purrr::map(merge_municipalities_helper,
...
merge_municipalities_helper <- function(rows,
                                        ...
                                        target_colnames) {
  rows %>%
    dplyr::summarise(comments = paste0("Retroactively (i.e. synthetically) merged from the municipalities ", pal::enum_str(municipality), "; ",
...
### Read in a Swiss canton's raw data files
...
read_raw_data_bern <- function(ballot_date) {
  ...
  if (is.na(file_type)) {
    ...
    cli::cli_alert_warning(msg_xlsx_csv_zip_file_missing)
...
-   Data might be spread over multiple files if a vote on the municipal level took place on the same date as votes on other levels. Before 2015-06-14, Swiss
    ...
    for specifics. The `read_raw_data_geneva()` function below reads in *all* of them as long as they're available. The `create_geneva_data()` function is
...
-   To determine the `group` there's in theory (i.e. according to the Genevan codebook) the variable `typelec` differentiating between *Suisse résident* (`SR`),
    *Suisse de l'étranger* (`SE`) and *Etranger résident* (`ER`). But de facto this variable is either always `NA` (up until 2009 and some later dates, e.g.
...
-   The variable `xnoref` (renamed to `id`) is a personal reference number *uniquely identifying the voter across all the raw data*.
...
read_raw_data_geneva <- function(ballot_date,
  ...
  if (length(filenames) == 0L) {
    ...
    cli::cli_abort("There is no raw data file available for Geneva @ {.val {ballot_date}}. See {.code swissevote:::metadata_geneva_raw_datasets}.")
  }
  ...
  if (anyNA(filenames)) {
    ...
    cli::cli_abort(paste0("The raw data filename for Geneva @ {.val {ballot_date}} ",
                          ifelse(length(na_notes) > 0L,
                                 "({.val {na_notes}}) ",
                                 ""),
                          "is unknown. Check {.code swissevote:::metadata_geneva_raw_datasets}."))
  }
  ...
  if (skip_problematic_dates &&
    ...
    cli::cli_alert_warning("Skipped processing data from {.val {ballot_date}} due to unresolved issues.")
...
read_raw_dataset_geneva <- function(filename,
  ...
  if (nr_of_na_municipality_code_2_digits > 0L) {
    ...
    cli::cli_alert_warning(paste0("{.val {nr_of_na_municipality_code_2_digits}} of a total of {nrow(data_raw)} entries in the raw Geneva data file {.file ",
                                  "{filename}} have no {.var municipality_code_2_digits} set ({.val {NA}}).",
                                  ifelse(remove_na_locality_codes,
  ...
  if (nr_of_na_country_canton_code > 0L) {
    ...
    ### print an informative warning
    cli::cli_alert_warning(paste0("{.val {nr_of_na_country_canton_code}} of a total of {nrow(data_raw)} entries in the raw Geneva data file {.file ",
                                  "{filename}} have no {.var country_canton_code} set ({.val {NA}}).",
                                  ifelse(remove_na_locality_codes,
                                         " They have been removed.",
                                         "")))
  ...
  if (length(affected_indices) > 0L) {
    ...
    cli::cli_alert_warning(paste0("There were {.val {length(affected_indices)}} entries in the raw Geneva data file {.file {filename}} that had a \"wrong\" ",
                                  "They have been fixed. The wrong district codes included the following numbers: {.val {affected_district_codes}}"))
  ...
  if (check_municipalities) {
    ...
    if (nrow(unknown_municipality_district_combos)) {
      ...
      cli::cli_alert_warning(paste0("For {.val {nrow(unknown_municipality_district_combos)}} of a total of {nrow(data_raw)} entries in the raw Geneva data ",
                                    "file {.file {filename}} no matching municipality could be found. This can either be due to flaws/errors in the data or ",
                                    "because {.var metadata_geneva_raw_datasets} needs an update.",
                                    ifelse(remove_unknown_municipality_district_combos,
                                           " The affected entries have been removed. To inspect them",
                                           " To inspect the affected entries"),
                                    ", enter {.code View(unknown_municipality_district_combos_GE_{filename}}). ",
                                    "The affected entries show the following combinations of {.var municipality_code_2_digits} and {.var district_code}:",
                                    "\n\n    ",
                                    unknown_municipality_district_combos %>%
                                      dplyr::select(municipality_code_2_digits, district_code) %>%
...
read_raw_data_neuchatel <- function(ballot_date) {
  ...
  if (data_raw %>%
    ...
    cli::cli_abort(paste0("The NE data file {.file {fs::path(file_path, ext = file_type)}} seems to contain empty surplus rows at the beginning. Please ",
  ...
  if (anyNA(data_raw$is_foreigner)) {
    
    cli::cli_alert_warning(paste0("Found {.val {length(which(is.na(data_raw$is_foreigner)))}} entries in raw data lacking a nationality ({.val {NA}}). Please ",
                                  "double-check raw data from {.val {ballot_date}}."))
  }
  ...
  if (data_raw %>%
    ...
    cli::cli_abort("Found Swiss abroad data entry in raw data with nationality other than Swiss. Please double-check data.")
  }
...
convert_canton_names <- function(cantons) {
  ...
  for (canton_name in checkmate::assert_character(cantons,
    ...
    if (!is.na(long_match)) {
      ...
      cli::cli_abort("{.val {canton_name}} is not a valid e-voting canton.")
    }
  }
...
ballot_dates_raw_data <- function(canton = c("Bern", "Geneva", "Neuchatel")) {
  ...
  result <-
    ...
    stringr::str_extract(pattern = ifelse(canton_short == "GE",
...
ballot_dates_federal <- function(source = c("Neuchatel", "Zurich"),
  ...
  pkgpins::with_cache(expr = {
    ...
    result <- switch(EXPR = source,
                     ...
                     Zurich =
                       get_zurich_rfrnd_dates(exclude_counterproposals = exclude_counterproposals) %>%
                       dplyr::filter(level == "federal") %>%
...
-   Note that Genevan Council of States election dates might have only `"Elections fédérales"` set as `subject` if the Council of States was elected at the sam
    date as the National Council (usually the case for the first round of voting).
  ...
  pkgpins::with_cache(expr = {
    ...
    for (canton in cantons) {
      ...
      switch(EXPR = canton,
             ...
             Zurich = cli::cli_alert_warning(paste0("For the canton of {.val {canton}} only referendum dates can be scraped. The cantonal election dates ",
                                                      "have to be gathered by hand.")))
      ...
      if (canton %in% c("Neuchatel", "Zurich")) {
        ...
        ballot_dates <-
          switch(EXPR = canton,
                 Geneva =
                   get_geneva_rfrnd_dates(exclude_counterproposals = exclude_counterproposals) %>%
                   dplyr::filter(level == "cantonal") %>%
                   dplyr::mutate(ballot_date,
                                 subject,
                                 is_election = FALSE,
        ...
        cli::cli_alert_warning("Scraping ballot dates from the canton of {.val {canton}} hasn't been implemented yet.")
      }
    }
