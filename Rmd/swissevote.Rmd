...
The Genevan vote register data is split among several files and oddly named. Thus we include the relevant metadata about it directly in this package. Below is
the code to generate the internal data from the CSV file (which is only included in the source repository but no the package).

```{r, purl = FALSE}
metadata_raw_geneva <- readr::read_csv(file = "metadata_raw_geneva.csv",
                                       col_types = "Dcllllicc")

usethis::use_data(metadata_raw_geneva,
                  internal = TRUE,
                  overwrite = TRUE,
                  compress = "xz",
                  version = 3)
...
- Data might be spread over multiple files if a vote on the municipal level took place on the same date as votes on other levels. Before 2015-06-14, Swiss
  abroad are also located in separate files (with filenames ending in the "virtual" municipality code `_46`). See `swissevote:::metadata_raw_geneva` for
  specifics. The `read_raw_data_geneva()` function below reads in _all_ of them as long as they're available. The `create_geneva_data()` function is designed
  to do a subsequent cross-vote aggregation.
...
- There seems to be more than one variable for multiple relevant entities in the Genevese voting register data. Therefore we had to decide which ones of these
  we use for our data processing:
    ...
    ------ | ------------------ | --------------- | -----
    `municipality` | `ccom`, `domicom`, `votcom` | **`votcom`** | in the `2016-11-27` data `ccom != votcom` in 27 instances and `domicom != votcom` in one instance
...
# Scrape vote dates from the authorities' websites

TODO: Completely overhaul the architecture to scrape all the sites, reducing unnecesary redundancy. Export a single fn `vote_dates()` that relies on internal
...
TODO: Meanwhile all the vote dates were migrated to the new page format (see `scrape_geneva_referendum_date_after_mid_2018()`) and all the GE scraping code
      needs to be updated.
